...
static const CGFloat kPreviewImageSize = 160;
static const CGFloat kDefaultCropWidth = 320;
static const CGFloat kDefaultCropHeight = 568;
...
- (void)viewDidLoad
{
    ...
    [self.view setBackgroundColor:[UIColor colorWithRed:67.0/255.0 green:67.0/255.0 blue:99.0/255.0 alpha:0]];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"SIZES BEFORE? %@", NSStringFromCGSize(self.imageView.frame.size));
    
    [self.imageView setImage:self.previewImage];
    
    NSLog(@"WHAT ARE THE SIZES OF THESE IMAGES?? %@ & %@", NSStringFromCGSize(self.imageView.frame.size), NSStringFromCGSize(self.previewImage.size));
...
- (void) setSourceImage:(UIImage *)sourceImage
{
    if( sourceImage != _sourceImage) {
        ...
        NSLog(@"AS SOON AS WE GET IT IT IS... %@", NSStringFromCGSize(_sourceImage.size));
